...
*
* href="http://www.counterpane.com">Counterpane Systems</a> have  by , John Kelsey, and . This precludes the
* possibility of using TEA as a hash function. Roger Needham and David Wheeler have proposed
* <a href="http://www.cl.cam.ac.uk/ftp/users/djw3/xtea.ps">extensions to TEA that
* counters the above attacks.</font>
...
//      Client client = new Client("localhost", 10000);
      String src = t.generatePlainText();//"hello world!";
      System.out.println("input = " + src);
      byte plainSource[] = src.getBytes();
      int enc[] = t.encode(plainSource, plainSource.length);
      System.out.println(t.padding() + " bytes added as padding.");
   ...
   * </PRE>
   *
   * @param v two <code>int array as input. 
   *
   ...
   */
   ...
   /**
   * Decipher two <code>ints.
   * Replaces the original contents of the parameters with the results.
   * The integers are usually decocted to 8 bytes.
   * The decoction of the <code>ints to bytes can be done
   * this way.
   * <PRE>
   * int x[] = decipher(ins);
   * outb[j]   = (byte)(x[0] >>> 24);
   ...
   *
   * @param b incoming <code>byte array
   *
   * @param byte count
   *
