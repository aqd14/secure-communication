 ...
 * Translator's notes:
 * <UL>
 * <LI>Although the this algorithm is optimised for 32-bit CPUs with fast shift
 * capabilities</I> Java manages to throw it all away by not providing unsigned
 * values resulting in the excessive use of AND's to prevent sign extension on
 * promotion of a byte to an integer.</LI>
 * <P>
 * <LI>The following description is taken from the Mach5 Software cryptography
 * archives at <A HREF="http://www.mach5.com/crypto/">www.mach5.com/crypto.
 * <p>
 ...
 * from <a
 *
 * href="http://www.counterpane.com">Counterpane Systems</a> have by , John
 * Kelsey, and . This precludes the possibility of using TEA as a hash function.
 * Roger Needham and David Wheeler have proposed
 * <a href="http://www.cl.cam.ac.uk/ftp/users/djw3/xtea.ps">extensions to TEA
...
public class TEA {
	...
	/**
	 * Encipher two <code>ints. Replaces the original contents of the parameters
	 * with the results. The integers are usually created from 8 bytes. The
	 * usual way to collect bytes to the int array is:
	 * 
	 * <PRE>
	 * byte ba[] = { .... };
	 * int v[] = new int[2];
	 ...
	 *
	 * @param v
	 *            two <code>int array as input.
	 *
	 * @return array of two <code>ints, enciphered.
	 */
	...
	/**
	 * Decipher two <code>ints. Replaces the original contents of the parameters
	 * with the results. The integers are usually decocted to 8 bytes. The
	 * decoction of the <code>ints to bytes can be done this way.
	 * 
	 * <PRE>
	 * int x[] = decipher(ins);
	 * outb[j] = (byte) (x[0] >>> 24);
	 ...
	 *            <code>int array of 2
	 *
	 * @return deciphered <code>int array of 2
	 */
	...
	/**
	 * Encipher two <code>bytess.
	 *
	 * @param v
	 *            <code>byte array of 2
	 *
	 * @return enciphered <code>byte array of 2
	 */
	 ...
	 *
	 * @return <code>byte array of decoded bytes.
	 */
